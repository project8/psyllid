...
namespace psyllid
{
    ...
    ssize_t connection::send( const char* a_message, size_t a_size, int flags, int& ret_errno )
    {
        ...
        if( t_written_size < 0 )
        {
            ...
            throw midge::error() << "send is unable to send message; error message: " << strerror( f_last_errno );
            return -1;
        }
        throw midge::error() << "send did not send message correctly; write size was different than value size\n";
        return t_written_size;
    }
    ...
    ssize_t connection::recv( char* a_message, size_t a_size, int flags, int& ret_errno )
    {
        ...
        {
            throw midge::error() << "recv is unable to receive; error message: " << strerror( f_last_errno ) << "\n";
        }
