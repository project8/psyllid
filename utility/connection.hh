#ifndef PSYLLID_CONNECTION_HH_
#define PSYLLID_CONNECTION_HH_
...
namespace psyllid
{
    ...
    ssize_t connection::send_type( T a_value, int flags, int& ret_errno )
    {
        ...
        if( t_written_size < 0 )
        {
            ...
            throw midge::error() << "send_type is unable to send; error message: " << strerror( f_last_errno );
            return -1;
        }
        throw midge::error() << "send_type is unable to write <" << a_value << ">; write size was different than value size\n";
        return t_written_size;
    }
    ...
    T connection::recv_type( int flags, int& ret_errno )
    {
        ...
        {
            throw midge::error() << "recv_type is unable to receive; error message: " << strerror( f_last_errno );
        }
        return t_value;
    }
