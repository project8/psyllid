...
// Packet payloads are divided into 64-bit integers, each of which is loaded in a little-endian way, and then transmitted over the network in big-endian order.
// During transmission the payload has this order: r3, i3, r2, i2, r1, i1, r0, i0, r7, i7, . . . .
// Therefore just using be64toh will end up reversing real and imaginary components when converting to a little-endian host, and have no effect at all on a big-endian host.
// We need to make this shift regardless of the byte ordering of the host, since we're not actually interpreting the payload as 64-bit integers, but as individual bytes.
     ...
     @details
        ...
        There are two kinds of packets: time-domain and frequency-domain.
          * Frequency domain:
             ...
             - The samples are grouped in 8-byte words with the following order in each word: R_3, I_3, R_2, I_2, R_1, I_1, R_0, I_0, where sample 0 is the lowest frequency in the word, and sample 3 the highest; the words are in canonical order.  Once the samples in each word are reordered, the first sample is DC and the last sample is (100MHz - channel width).
          ...
          * Time-domain:
             ...
             - The samples are grouped in 8-byte words with the following order in each word: I_3, Q_3, I_2, Q_2, I_1, Q_1, I_0, Q_0, where sample 0 is first in time, and sample 3 is last in time; the words are in temporal order.
